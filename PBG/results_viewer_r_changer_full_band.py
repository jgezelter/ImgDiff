...
results_2 = results.sort_values(by = ["r", "omega"])
results_2 = results_2[results_2["aniso"] < 0.1]
...
s1=ax[1].scatter(results_2["r"], results_2["omega"], c=abs(results_2["b"]), s=size, norm = "log")
s2=ax[2].scatter(results_2["r"], results_2["omega"], c=results_2["aniso"], s=size, norm = "log")
fig.supxlabel("Pillar Radius")
...
results_2[["r", "omega", "a", "b", "aniso"]].to_csv(f"{path}full_result.csv")

fig2, ax2= plt.subplots()
s20=ax2.scatter(results_2["r"], results_2["aniso"], c=results_2["a"], s=size, norm="log")
ax2.set_yscale("log")
ax2.set_xlim(0,0.5)
ax2.set_xlabel("Pillar Radius")
ax2.set_ylabel("Anisotropy")
ax2.set_title("a")
fig2.colorbar(s20, ax=ax2)

fig2.savefig(f"{path}full_result_2.png")
...
decay=int(sys.argv[9])

resolution = 100 # fdtd grid resolution
...
dpad = 5 # thickness of glass above/below PhC
...
k_point = mp.Vector3(0, 0, 255/300 * 1).rotate(mp.Vector3(0, 1, 0), theta_r)
...
geometry_trial = [
    mp.Block(
        material=mp.Medium(index=1.48),
        size=mp.Vector3(sx,sy,ph*3),
        center=mp.Vector3(0,0,ph*1.5)
    ),
    mp.Block(
        material=mp.Medium(index=1.45),
        size=mp.Vector3(sx,sy,sz/2),
...
if polarization_s:
    sources = [
        mp.Source(
            mp.GaussianSource(237.5/300,fwidth=2,cutoff=10), 
            component = mp.Ey, 
            center = src_pt,
        ...
        mp.Source(
            mp.GaussianSource(255/300,fwidth=2,cutoff=10), 
            component = mp.Ey, 
            center = src_pt,
            size = mp.Vector3(sx, sy, 0), 
            amp_func=pw_amp(k_point, src_pt)
        ...
        mp.Source(
            mp.GaussianSource(272.5/300,fwidth=2,cutoff=10), 
            component = mp.Ey, 
            center = src_pt,
            size = mp.Vector3(sx, sy, 0), 
            amp_func=pw_amp(k_point, src_pt)
...
if polarization_s:
    sim_base.run(until_after_sources=mp.stop_when_fields_decayed(dt=0.05,c=mp.Ey,pt=trans_mon_pt,decay_by=10**(-1*decay)))
else: 
    sim_base.run(until_after_sources=mp.stop_when_fields_decayed(dt=0.05,c=mp.Hy,pt=trans_mon_pt,decay_by=10**(-1*decay)))
...
if polarization_s:
    sim.run(until_after_sources=mp.stop_when_fields_decayed(dt=0.05,c=mp.Ey,pt=trans_mon_pt,decay_by=10**(-1*decay)))
else: 
    sim.run(until_after_sources=mp.stop_when_fields_decayed(dt=0.05,c=mp.Hy,pt=trans_mon_pt,decay_by=10**(-1*decay)))


t_flux = mp.get_fluxes(trans_flux)
Tflux = np.divide(t_flux, b_flux)
...
fig.savefig(f"{path}{angle_deg}.png")

out_data = {
    "angle_deg": [angle_deg]*len(Tflux), 
    "freqs": mp.get_flux_freqs(trans_flux), 
    "trans": Tflux
}

out = pd.DataFrame(out_data)
out.to_csv(f"{path}{angle_deg}.csv", index = False)

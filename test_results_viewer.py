...
results = pd.concat([pd.read_csv(f) for f in files], ignore_index=True).set_axis(["freq", "trans"], axis = 1).sort_values(by="freq")
...
fig,ax=plt.subplots()
ax.plot(freq,trans)
#ax.scatter(freq,trans, marker="o",facecolors="none", edgecolors="r")
plt.xlabel("\u03c9")
plt.xlim(0.11,0.67)
plt.ylabel("t(\u03c9)")
plt.ylim(0,1)
